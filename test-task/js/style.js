...
function FindOnPage(name, status) {
        ...
        if(input.length>=3)
	{
		     ...
		     result_arr = [];   //в этом массиве будем хранить результат работы (подсветку)
		        ...
		        result_arr[i] = result[i].replace(eval(search), '<span style="background-color:black;">'+input+'</span>'); //находим нужные элементы, задаем стиль и сохраняем в новый массив
			}
		     for(var i=0; i<result.length;i++) {
